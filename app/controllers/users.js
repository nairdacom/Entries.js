...
var Users = function () {
  ...
  this.lockChange = function(req,resp,params){
    var self = this;
    this.currentUser = this.session.get('user');
    geddy.model.User.first(params.userId,function(err,data){
      if(data.isLockedOut) data.isLockedOut = false;
      else data.isLockedOut = true;
      data.save(function(err,out){
        if (!err) self.redirect("/users/");
      });
    });
  };
  ...
  this.index = function (req, resp, params) {
    ...
    this.currentUser = this.session.get('user');
    if(!this.currentUser.isAdmin) this.redirect('/');
    var self = this;
  ...
  this.edit = function (req, resp, params) {
    ...
    geddy.model.User.first(params.id, function(err, user) {
      ...
      if (!user) {
        ...
        self.respondWith(user,self.currentUser);
      }
    });
  };
